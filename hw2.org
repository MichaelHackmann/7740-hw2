...
grammar_string = """
    S -> S1 CONJ S2
    S1[NUM=?n, PER=?p] -> NP[NUM=?n, PER=?p] VP[NUM=?n, PER=?p]
    S2[NUM=?n, PER=?p] -> NP[NUM=?n, PER=?p] VP[NUM=?n, PER=?p]
    ...
    N[NUM='sg', PER='3'] -> 'dog' | 'toy' | 'elf' | 'ride' | 'country' | 'week' | 'pajamas' | 'child' | 'chair' | 'back' | 'coffee' | 'walk' | 'park' | 'head' | 'pin'
    N[NUM='pl', PER='3'] -> 'dogs' | 'toys' | 'angels' | 'chairs' | 'coffees' | 'walks' | 'parks' | 'heads' | 'pins' | 'rides'

    V[NUM='sg', PER='3'] -> 'sleeps' | 'plays' | 'sees' | 'is' | 'was' | 'might' | 'has' | 'barks' | 'slept' | 'stretched' | 'made' | 'had' | 'played' | 'be' | 'playing'
    V[NUM='pl', PER='3'] -> 'are' | 'play' | 'take' | 'dancing' | 'sees' | 'slept' | 'stretched' | 'made' | 'played' | 'be' | 'playing'

    AV -> 'quietly' | 'today' | 'tomorrow' | 'not' | 'downstairs'
    P -> 'by' | 'with' | 'to' | 'on' | 'of' | 'in' | 'next'
    CONJ -> 'and' | 'but'
    Num -> 'one' | 'two' | 'three' | 'four' | 'five' | 'six' | 'seven' | 'eight' | 'nine' | 'ten' | '49' 
"""

feature_grammar = FeatureGrammar.fromstring(grammar_string)
cp = FeatureChartParser(feature_grammar)
...
for sentence in sentences:
    ...
    parsed_trees = list(cp.parse(sentence.split()))  
    if parsed_trees:
...
When looking at the results of the parsing, it seems like the parser operated efficietly with the grammar rules given
to it. The ones that we used in the NLTK script have accurate breakdowns of the sentences. The parser did show very limited
flexibility in our opinion. It struggled heavily with sentence structures that came from the pre defined grammar. This dependency
on rules highlights a challenge that a parser would need to handle a huge amount of sentence constructions at the same time and quick.
The need to define every single possible grammar structure before parsing seems impossible on a high scale application. The
parser is good for small projects or problems, but would be heavily difficult to use in large scale projects or real world applications. 
